...
- **Game Record (`game_record.hpp`):** The test suite verifies `GameRecord` navigation (`next`/`prev` edge cases) and `loadPgn`. Additional coverage could explore deep history branching.

### `src/engine/` (Stockfish Integration & Analysis)

- **Engine Tests:** Basic `StockfishClient` integration is tested, relying on a real background process in `test_stockfish_integration()`. However, `game_reviewer.cpp` and `game_reviewer.hpp` are currently untested.
- **Recommendation:** Tests should mock the input/output Win32 pipes to ensure that the engine class correctly parses evaluation metrics (centipawns, mates) and best move suggestions from UCI format without relying on a real background process during unit testing. This would also allow core testing on Linux without Windows pipes. `GameReviewer` should be tested against static FENs and mock engine evaluations to verify classification (Blunder, Inaccuracy, GameEnd) and ACPL accuracy math independently.
