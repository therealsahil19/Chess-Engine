...
- **Game State Tests:** The current tests only cover `isInsufficientMaterial`, basic `isCheck`, and `isCheckmate`. Tests should be added explicitly for **Stalemate** scenarios with specifically crafted FENs.
...
- **Game Record (`game_record.hpp`):** Game history navigation, including backwards and forwards movement, is now tested via `addMove()`.

### `src/engine/` (Stockfish Integration & Analysis)

- **No Engine Tests:** `stockfish.cpp`, `stockfish.hpp`, `game_reviewer.cpp`, and `game_reviewer.hpp` are currently untested.
