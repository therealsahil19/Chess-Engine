...
- **Game State Tests:** Tests exist for `isInsufficientMaterial`, `isCheck`, `isCheckmate`, and board parsing (`loadFen`, `loadPgn`). However, tests should be added explicitly for **Stalemate** scenarios with specifically crafted FENs.
...
- **Game Record (`game_record.hpp`):** Basic undo/redo and addMove logic is tested, but extensive tests navigating the game history accurately beyond simple state transitions could be expanded.

### `src/engine/` (Stockfish Integration & Analysis)

- **Engine Tests:** Basic integration for `StockfishClient` starting, position setup, and callback firing is tested. `GameReviewer` move classification and accuracy summary math is tested.
- **Recommendation:** `StockfishClient` tests could be expanded to mock the input/output Win32 pipes to ensure that the engine class correctly parses evaluation metrics (centipawns, mates) from UCI format without relying on a real background process during unit testing.
